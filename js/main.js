...
function updateMenuStats() {
  const t = document.getElementById('statTaps');
  const c = document.getElementById('statChains');
  if (t) t.textContent = String(save.taps | 0);
  if (c) c.textContent = String(save.chains | 0);
...
function showWin() {
  setScreen('win');
  document.getElementById('winTaps').textContent = String(sessionTaps);
  document.getElementById('winAll').textContent = String(save.chains | 0);
  if (window.__pendingReload) {
    window.__pendingReload = false;
    window.__reloaded = true;
    location.reload();
...
/** Reload when idle; defer mid-play so a round isn't interrupted. */
